...
public class LoginController { // class start
    ...
    public boolean login(@RequestBody LoginDto loginDto , HttpServletRequest request){
        ...
        System.out.println( request.getHeader("User-Agent") ); // 통신을 요청한 클라이언트(사용자)의 브라우저정보 확인 , HTTP Header
        // **** 세션 정보 가져오기 , 세션이란? 톰캣 서버내 저장소 ( key-value / MAP 컬렉션 사용 )
        HttpSession session = request.getSession();
        System.out.println( session.getId() ); // 세션의 식별 번호 : 브라우저 마다 , pc마다 다르게 할당 된다. 즉] 사용자 마다의 개인 저장소
