...
public class RestController1 {// class start
    ...
    public boolean method2( @RequestParam( name = "name" , defaultValue = "홍길동" , required = false) String name ){
        ...
        // ++ required : 만약에 해당 매개변수가 없으면 예외 (자동) 발생 , HTTP 400 ERROR(잘못된 요청 뜻)
            ...
            // 예] http://localhost:8080/day03/param             ---> 400 ERROR
        System.out.println("RestController1.method2"); // soutm
        System.out.println("mane = " + name); // soutp 현재 메소드 매개변수 출력
        return true;
    }// func end

    @GetMapping("/day03/param2") //
    public int method3( @RequestParam( required = true ) String name , @RequestParam(name = "age") int 나이 ){
    ...
    public String method4( @RequestParam Map<String , String> map){
        ...
        // Map : KET 와 VALUE를 한쌍(ENTRY) 구성하여 여러개 ENTRY 저장 구조
    ...
    // ++ DTO 에 자동 매핑 하기 위해서는 DTO에도 name 과 age 멤버변수가 존재 해야한다. +생성자+setter/getter+toString()
    ...
    * 2) BODY(본문)이란 ? HTTP본문에 매개변수 표현 , 매개변수 노출X , 보안 안전 , POST/PUT 에서 JSON타입으로 객체 지원
    ...
    역할          단일 파라미터 변수 바인딩                      복수 파라미터에 객체 바인딩                                             본문(body) 객체 바인딩
