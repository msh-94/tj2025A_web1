...
public class FileService {// class start
    ...
    // 방법2) 프로젝트내 build : 서버실행했을때 컴파일된 코드 즉] 내장서버가(24시간) 갖는 실행된 폴더

    // 1. 현재 프로젝트의 최상위 디렉토리(폴더) 경로 찾기
    private String baseDir = System.getProperty("user.dir");
    // 2. 방법2 처럼 개발자폴더가 아닌 실행된서버의 폴더로 업로드 경로 지정하기 , *개발환경*에 따라 달라진다
    ...
    public String fileUpload(MultipartFile multipartFile){
        ...
        // 이유 : uuid 와 파일명 구분을 _언더바 사용하기 위한 내부적인 규칙 , .replaceAll("기존문자" , "새로운문자) : 기존문자를 새로운문자로 변환 메소드
    ...
    public void fileDownload(String fileName , HttpServletResponse response){
        ...
        // ************************ 다운로드 형식 지정 : 브라우저 마다 상이 *********************** //
            ...
            String oldFileName = fileName.split("_")[1]; // UUID_파일명 , _언더바 기준으로 잘라서 2번째 문자열 뜻
        ...
        // 4. 업로드할 파일을 사용자에게 (response객체 이용한) 응답하기
            ...
            // 4-2 '3-3' 에서 읽어온 바이트 배열을 다운로드 요청한(response) 사용자에게 출력/쓰기 하기
            fout.write(bytes);
            // 4-3 안전하게 스트림 닫기
            fout.close(); // 필수는 아니지만 대용량에서는 안전하게 스트림을 직접 닫기 권장!
