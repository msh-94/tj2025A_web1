...
const func7 = ( ) => { console.log( 'func7 exe' );
    ...
    fetch("/day04/exam7" , option )
        .then( r => r.json() )              // + then/catch 함수내 매개변수명은 임의로 한다
...
/*
    [FETCH] 라이브러리 (내장)함수
        - 외부 주소로 부터 요청 과 응답을 *비동기 통신* 지원하는 함수 , * 비동기통신 : fetch , ajax(JQUERY) , Axios(React)
        - 사용법
            fetch( URL , HEADER ).then( response => response.타입명() ).then( data => { 실행문; } ).catch( error => { 실행문; } );
                1) 통신할 URL 주소    
                2) 통신할 option
                    {
                        method : "POST or PUT"
